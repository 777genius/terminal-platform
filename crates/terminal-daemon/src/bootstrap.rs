...
use crate::{
    TerminalDaemon,
    state::{TerminalDaemonBackendConfig, TerminalDaemonState, TerminalDaemonStateBuildError},
};

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct TerminalDaemonBootstrapConfig {
    #[serde(default = "compiled_default_backends")]
    pub enabled_backends: Vec<BackendKind>,
}

impl Default for TerminalDaemonBootstrapConfig {
    fn default() -> Self {
        Self { enabled_backends: compiled_default_backends() }
    }
}
...
impl TerminalDaemonBootstrapConfig {
    ...
    pub fn build_state(&self) -> Result<TerminalDaemonState, TerminalDaemonStateBuildError> {
        TerminalDaemonState::with_backend_config(self.backend_config())
    }

    pub fn build_daemon(&self) -> Result<TerminalDaemon, TerminalDaemonStateBuildError> {
        self.build_state().map(TerminalDaemon::new)
    }
}
...
fn compiled_default_backends() -> Vec<BackendKind> {
    TerminalDaemonState::compiled_backends()
}
...
mod tests {
    ...
    use super::{
        TerminalDaemonBootstrapConfig, TerminalDaemonBootstrapConfigError,
        compiled_default_backends,
    };
    ...
    fn default_backends_track_compiled_backends() {
        ...
        assert_eq!(config.enabled_backends, compiled_default_backends());
    }
