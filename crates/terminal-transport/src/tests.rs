...
use terminal_protocol::{
    ListSessionsResponse, LocalSocketAddress, RequestEnvelope, RequestPayload, ResponseEnvelope,
    ResponsePayload, SubscriptionEvent, SubscriptionRequest, SubscriptionRequestEnvelope,
...
use super::{
    LocalSocketTransportClient, TransportBoxFuture, TransportRequestHandler, TransportSubscription,
    TransportSubscriptionHandler, spawn_local_socket_server,
};
...
async fn request_reply_roundtrips_through_transport_handlers() {
    ...
    let response =
        client.send_request(RequestPayload::ListSessions).await.expect("request should succeed");
...
async fn subscription_lane_opens_receives_and_closes_explicitly() {
    ...
    let server = spawn_local_socket_server(
        StubHandler { cancel_observed: Some(cancel_observed.clone()), emit_initial_event: true },
        address.clone(),
    )
    .expect("server should bind");
    let client = LocalSocketTransportClient::new(address);
...
async fn server_can_restart_on_the_same_address() {
    ...
    let first = spawn_local_socket_server(StubHandler::default(), address.clone())
        .expect("server should bind");
    client.send_request(RequestPayload::ListSessions).await.expect("first request should succeed");
    ...
    client.send_request(RequestPayload::ListSessions).await.expect("second request should succeed");
