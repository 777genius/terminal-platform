...
#[cfg(target_os = "linux")]
use crate::support::detect_linux_libc;
use crate::{
    constants::*,
    support::{assert_value, copy_file, file_name, read_json, workspace_root, write_json},
};

pub(crate) fn stage_capi_package(out_dir: &Path) -> Result<PathBuf, String> {
...
pub(crate) fn verify_capi_package(package_dir: &Path) -> Result<(), String> {
...
pub(super) fn current_target_descriptor() -> serde_json::Value {
    serde_json::json!({
