...
impl TerminalPersistenceV2 {
    pub fn import_saved_native_session_snapshot(
        ...
        self.import_saved_native_session_snapshot_with_connection(&mut connection, saved)
    }

    pub(crate) fn import_saved_native_session_snapshot_with_connection(
        &self,
        ...
        let import_result =
            self.import_saved_native_session_snapshot_with_writer(connection, saved, &lease.id);
        let release_result = self.release_writer_generation_with_connection(connection, &lease.id);

        match (import_result, release_result) {
            (Ok(()), Ok(())) => {
                self.restore_plan_with_connection(connection, &saved.session_id.0.to_string())
            }
    ...
    fn import_saved_native_session_snapshot_with_writer(
        ...
        self.upsert_legacy_visual_session_with_connection(connection, saved)?;
        for screen in &saved.screens {
            self.upsert_legacy_visual_pane_with_connection(connection, saved, screen)?;
            self.write_screen_snapshot_with_connection(
                connection,
                ScreenSnapshotInput {
                    id: None,
        ...
        self.write_topology_snapshot_with_connection(
            connection,
