...
import { TerminalWorkspaceGatewayServer } from "../dist/features/terminal-workspace/main/adapters/input/TerminalWorkspaceGatewayServer.js";
import {
...
import { deriveTerminalDemoSessionStreamUrl } from "../dist/features/terminal-workspace/contracts/index.js";
...
test("gateway exposes opaque import handles instead of foreign backend routes", async () => {
  ...
  const gateway = await TerminalWorkspaceGatewayServer.start({
    runtimeSlug: "terminal-demo",
    controlService: new TerminalWorkspaceControlService(runtime),
    sessionStreamService: new TerminalWorkspaceSessionStreamService(runtime),
  });
...
test("gateway keeps session state traffic on the stream plane only", async () => {
  ...
  const streamClient = createStreamClient(deriveTerminalDemoSessionStreamUrl(gateway.controlPlaneUrl));
  try {
