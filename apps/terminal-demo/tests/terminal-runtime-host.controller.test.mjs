...
} from "../dist/features/terminal-workspace-kernel/contracts/index.js";
import {
...
test("selectSession degrades to session error when subscription start fails", async () => {
  const store = createStore();
  const gateway = createGateway({
    subscribeError: new Error("stream attach failed"),
  });
  const controller = new TerminalRuntimeWorkspaceController(gateway, gateway, store);
  ...
  assert.equal(store.getState().sessionStreamHealth.lastError, "stream attach failed");
});
...
test("session stream reconnecting state is explicit while keeping the last snapshot", async () => {
  ...
  assert.deepEqual(state.sessionStreamHealth, {
    phase: "reconnecting",
    reconnectAttempts: 2,
    lastError: "socket dropped",
  });
