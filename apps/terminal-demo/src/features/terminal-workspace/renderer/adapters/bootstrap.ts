import {
  deriveTerminalDemoSessionStreamUrl,
  type TerminalDemoBootstrapConfig,
} from "../../contracts/index.js";

interface BootstrapResolution {
  config: TerminalDemoBootstrapConfig | null;
  error: string | null;
}

export function resolveTerminalWorkspaceBootstrapConfig(): BootstrapResolution {
...
function normalizeBootstrapConfig(
  raw:
    ...
    | null
    | undefined,
): TerminalDemoBootstrapConfig | null {
  if (!raw) {
    return null;
  }
  ...
  const sessionStreamUrl = raw.sessionStreamUrl?.trim()
    ?? (controlPlaneUrl ? deriveTerminalDemoSessionStreamUrl(controlPlaneUrl) : null);
