import type {
  ...
  TerminalRuntimeCreateSessionDraft,
  TerminalRuntimeWorkspaceState,
...
export class TerminalRuntimeWorkspaceController {
  ...
  async createNativeSession(input: TerminalRuntimeCreateSessionDraft): Promise<void> {
    await this.runAction(async () => {
      const created = await this.#controlPlane.createNativeSession(
        buildCreateNativeSessionPayload(input),
      );
  ...
  private patchSessionStreamHealth(health: TerminalWorkspaceSessionStreamHealth): void {
    const nextPatch: Partial<TerminalRuntimeWorkspaceState> = {
