...
The browser runner prints a `TERMINAL_DEMO_BROWSER_URL=...` line. Open that URL in Chrome or another browser. Browser mode uses a temporary session store by default so repeated demos start clean. Set `TERMINAL_DEMO_SESSION_STORE_PATH=/path/to/session-store.sqlite3` to inspect a specific store, or `TERMINAL_DEMO_BROWSER_PERSIST_SESSION_STORE=1` to use the default persistent daemon store.
